...
DERP_PATH=$PWD
TAG="android-12.0.0_r${1:?}"
SRC="12"

do_not_merge="vendor/derp manifest updater packages/apps/OmniSwitch packages/apps/OmniStyle \
packages/apps/OwlsNest external/google packages/apps/Launcher3 hardware/qcom/power \
packages/apps/Gallery2 device/qcom/common device/qcom/sepolicy device/derp/sepolicy \
...
# DerpFest manifest is setup with repo path first, then repo name, so the path attribute is after 2 spaces, and the path itself within "" in it
while read -r repos; do
    ...
    if [[ ${do_not_merge} =~ ${repos} ]]; then
        ...
        if ! git fetch aosp --quiet --tags; then
            echo "$repos" >> "${DERP_PATH}"/notaosp
        else
            if ! git merge "${TAG}" --no-edit; then
                echo "$repos" >> "${DERP_PATH}"/failed
                echo "$red $repos failed :( $end"
            else
                if [[ "$(git rev-parse HEAD)" != "$(git rev-parse derp/${SRC})" ]]; then
                    echo "$repos" >> "${DERP_PATH}"/success
...
done < <(grep 'remote="derp"' "${DERP_PATH}"/.repo/manifests/snippets/derp.xml | awk '{print $2}' | awk -F '"' '{print $2}' | grep -v caf)

echo "Repos that failed are - "
cat "$DERP_PATH"/failed
